...
export const Isometric3DViewer: React.FC<Isometric3DViewerProps> = ({ task }) => {
  ...
  return (
    <div className="bg-[#0B132B] border border-orange-900/40 rounded-xl overflow-hidden shadow-2xl flex flex-col">
      {/* 3D Viewport Header Bar */}
      <div className="bg-[#101B38] border-b border-slate-800 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center space-x-2">
          <div className="w-2.5 h-2.5 rounded-full bg-orange-500 animate-ping" />
          <span className="text-xs font-mono tracking-wider text-orange-400 font-bold uppercase">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`text-xs px-2.5 py-1 rounded-md font-medium transition flex items-center space-x-1.5 ${
              showStressMap
                ? 'bg-gradient-to-r from-red-600 to-amber-500 text-white shadow-lg shadow-orange-500/20'
          ...
          {/* View angle toggles */}
          <div className="bg-slate-900 p-0.5 rounded-lg flex border border-slate-800 text-[11px] font-mono">
            <button
              onClick={() => setViewAngle('iso')}
              className={`px-2 py-0.5 rounded ${viewAngle === 'iso' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              İzometrik
            </button>
            <button
              onClick={() => setViewAngle('front')}
              className={`px-2 py-0.5 rounded ${viewAngle === 'front' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Ön
            </button>
            <button
              onClick={() => setViewAngle('top')}
              className={`px-2 py-0.5 rounded ${viewAngle === 'top' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Üst
            </button>
          </div>
      ...
      {/* SVG 3D Canvas Area */}
      <div className="relative bg-[#060A14] min-h-[360px] h-[400px] w-full flex items-center justify-center p-4 overflow-hidden">
        {/* Subtle radial glow background */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-orange-950/20 via-slate-950 to-[#060A14] pointer-events-none" />
        ...
        <svg viewBox="0 0 600 380" className="w-full h-full max-w-[560px] select-none">
          <defs>
            ...
            <linearGradient id="topFace" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor={showStressMap ? '#ef4444' : '#475569'} />
          ...
          {viewAngle === 'iso' && (
            <g transform="translate(180, 70)">
              ...
              {/* Hole 1 - Top Vertical */}
              <ellipse cx="170" cy="80" rx="10" ry="5" fill="#060A14" stroke="#38bdf8" strokeWidth="1.5" />
              
              {/* Hole 2 - Bottom Base */}
              <ellipse cx="255" cy="130" rx="12" ry="6" fill="#060A14" stroke="#38bdf8" strokeWidth="1.5" />
            </g>
          )}
          ...
          {viewAngle !== 'iso' && (
            <g transform="translate(200, 90)">
              ...
              <circle cx="70" cy="70" r="16" fill="#060A14" stroke="#f97316" strokeWidth="2" />
              <circle cx="170" cy="70" r="16" fill="#060A14" stroke="#f97316" strokeWidth="2" />
              <circle cx="70" cy="150" r="16" fill="#060A14" stroke="#f97316" strokeWidth="2" />
              <circle cx="170" cy="150" r="16" fill="#060A14" stroke="#f97316" strokeWidth="2" />
            </g>
          )}
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  cy={cy}
                  r={isSelected ? '16' : '12'}
                  fill={isSelected ? 'rgba(249, 115, 22, 0.3)' : 'rgba(14, 165, 233, 0.2)'}
        ...
        {selectedAnnotation && (
          <div className="absolute bottom-4 left-4 right-4 md:right-auto md:max-w-md bg-[#0F172A]/90 backdrop-blur border border-orange-500/50 rounded-lg p-3 shadow-xl">
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-2">
                <span className="bg-orange-950 text-orange-400 border border-orange-700/50 text-[10px] font-mono px-2 py-0.5 rounded font-bold">
      ...
      <div className="bg-[#0c1428] border-t border-slate-800 p-2 flex items-center space-x-2 overflow-x-auto">
