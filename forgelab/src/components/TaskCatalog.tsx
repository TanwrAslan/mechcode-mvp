...
export const TaskCatalog: React.FC<TaskCatalogProps> = ({
  ...
  const getDifficultyBadge = (difficulty: TaskDifficulty) => {
    switch (difficulty) {
      ...
      case 'Orta':
        return <span className="text-[10px] font-bold text-[#FF6B00] uppercase bg-[#FF6B00]/10 border border-[#FF6B00]/30 px-2 py-0.5 rounded font-mono">Orta</span>;
      case 'İleri':
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[#30363D] pb-6">
        ...
        <div className="bg-[#161B22] border border-[#30363D] p-3 rounded-xl flex items-center space-x-3">
          ...
          <button
            ...
            className="text-xs bg-[#FF6B00] text-black hover:bg-[#e66000] px-3 py-1.5 rounded font-bold transition uppercase tracking-wider"
          >
            PRO'ya Geç
          </button>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded text-xs font-medium transition whitespace-nowrap ${
                selectedDifficulty === diff
                  ? 'bg-[#FF6B00] text-black font-bold shadow-[0_0_10px_rgba(255,107,0,0.2)]'
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-[#0D1117] border border-[#30363D] rounded-lg text-xs text-white placeholder-[#8B949E] focus:outline-none focus:border-[#FF6B00] transition"
          />
        </div>
      </div>
            ...
            <div
              ...
              className={`group bg-[#161B22] border rounded-xl p-5 transition duration-200 flex flex-col justify-between relative overflow-hidden cursor-pointer ${
                isLocked
                  ...
                  : 'border-[#30363D] hover:border-[#FF6B00]/70 hover:bg-[#21262D] shadow-lg'
              }`}
            >
              ...
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  {isLocked ? (
                    <span className="bg-[#21262D] text-[#8B949E] border border-[#30363D] text-[10px] font-mono px-2 py-0.5 rounded flex items-center space-x-1 font-bold">
                      <LockKeyhole className="w-3 h-3 text-[#FF6B00]" />
                      <span>KİLİTLİ</span>
                    </span>
                    ...
                    <span className="bg-[#FF6B00]/10 text-[#FF6B00] border border-[#FF6B00]/30 text-[10px] font-mono px-2 py-0.5 rounded font-bold">
                      MEVCUT
                    </span>
                  )}
                </div>
                ...
                {/* Task Title */}
                <h3 className="text-lg font-bold text-white group-hover:text-[#FF6B00] transition-colors leading-snug">
                  {task.title}
                </h3>
              ...
              <div className="mt-6 pt-4 border-t border-[#30363D] flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1 text-xs font-bold text-[#FF6B00] group-hover:translate-x-1 transition-transform">
