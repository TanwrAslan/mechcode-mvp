...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, requireAdmin = false }) => {
  ...
  if (loading) {
    return (
      <FullScreenMessage>
        <div className="flex items-center gap-3 text-[#8B949E] text-sm font-mono">
  ...
  if (requireAdmin && !isAdmin) {
    return (
      <FullScreenMessage>
        <div className="max-w-md text-center border border-[#EF4444]/40 bg-[#EF4444]/5 rounded-lg p-8">
          <ShieldAlert className="w-10 h-10 text-[#EF4444] mx-auto mb-4" />
