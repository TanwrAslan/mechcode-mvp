    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-6 sm:p-8 space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-4">
            <div className="w-16 h-16 rounded bg-[#EF4444] flex items-center justify-center font-extrabold text-2xl text-black font-mono shrink-0 shadow-[0_0_15px_rgba(239,68,68,0.3)]">
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto">
            <button
              ...
              className="w-full md:w-auto px-6 py-3 rounded bg-[#EF4444] hover:bg-[#DC2626] text-black font-extrabold text-xs shadow-[0_0_15px_rgba(239,68,68,0.25)] transition flex items-center justify-center space-x-2 shrink-0 uppercase tracking-wider"
            >
        ...
        <div className="pt-6 border-t border-[#30363D] grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[#0D1117] p-3 rounded border border-[#30363D]">
            ...
            <span className="text-xl font-bold text-[#EF4444] font-mono mt-0.5 block">
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-4 flex items-center justify-between gap-4 text-xs">
        <div className="flex items-center space-x-3">
          <Sparkles className="w-4 h-4 text-[#EF4444] shrink-0" />
        ...
        <button
          onClick={handleCopy}
          className="bg-[#EF4444] text-black px-3.5 py-1.5 rounded font-bold hover:bg-[#DC2626] transition shrink-0 flex items-center space-x-1 uppercase text-xs"
        >
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-6">
          {completedTasks.map((task) => (
            <div
              key={task.id}
              className="bg-[#161B22] border border-[#30363D] hover:border-[#EF4444]/50 rounded-xl p-6 transition space-y-6"
            >
              {/* Card Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#30363D] pb-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="text-xs bg-[#EF4444]/10 text-[#EF4444] hover:bg-[#EF4444] hover:text-black px-3 py-1.5 rounded font-bold transition border border-[#EF4444]/30 flex items-center space-x-1 uppercase"
                  >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs font-mono">
                ...
                <div className="bg-[#0D1117] p-3 rounded border border-[#30363D] flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <span className="text-[#EF4444] font-bold">{task.exampleSolution.weightReduction}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0D1117] p-3.5 rounded border border-[#30363D] space-y-2 text-xs">
                ...
                <ul className="list-disc list-inside space-y-1 text-[#8B949E]">
                  {task.exampleSolution.designDecisions.map((dec, i) => (
                    <li key={i}>
                      <strong className="text-[#EF4444]">{dec.title}:</strong> {dec.explanation}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            ...
            <div className="bg-[#0D1117] p-3 rounded border border-[#30363D] flex items-center justify-between font-mono text-xs">
              <span className="text-[#EF4444] truncate mr-2">{shareableUrl}</span>
              <button
                onClick={handleCopy}
                className="bg-[#EF4444] text-black px-3 py-1.5 rounded font-bold hover:bg-[#DC2626] transition shrink-0 flex items-center space-x-1 uppercase"
              >
                {copiedLink ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
      ...
      {reportModalTask && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-6 max-w-md w-full space-y-4 shadow-2xl text-center">
            <div className="w-10 h-10 rounded-full bg-[#EF4444]/20 border border-[#EF4444]/40 flex items-center justify-center text-[#EF4444] mx-auto">
            ...
            <button
              ...
              className="w-full py-2.5 rounded bg-[#EF4444] text-black font-extrabold text-xs hover:bg-[#DC2626] transition uppercase"
            >
              Tamam
            </button>
          </div>
