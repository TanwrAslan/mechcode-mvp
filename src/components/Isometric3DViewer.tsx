...
export const Isometric3DViewer: React.FC<Isometric3DViewerProps> = ({ task }) => {
  ...
  return (
    <div className="bg-[#0B132B] border border-red-900/40 rounded-xl overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="bg-slate-900 p-0.5 rounded-lg flex border border-slate-800 text-[11px] font-mono">
            <button
              ...
              className={`px-2 py-0.5 rounded ${viewAngle === 'iso' ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              İzometrik
            </button>
            <button
              onClick={() => setViewAngle('front')}
              className={`px-2 py-0.5 rounded ${viewAngle === 'front' ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Ön
            </button>
            <button
              onClick={() => setViewAngle('top')}
              className={`px-2 py-0.5 rounded ${viewAngle === 'top' ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Üst
            </button>
          </div>
      ...
      <div className="relative bg-[#060A14] min-h-[360px] h-[400px] w-full flex items-center justify-center p-4 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-red-950/20 via-slate-950 to-[#060A14] pointer-events-none" />
        ...
        <svg viewBox="0 0 600 380" className="w-full h-full max-w-[560px] select-none">
          ...
          {viewAngle !== 'iso' && (
            <g transform="translate(200, 90)">
              ...
              <circle cx="70" cy="70" r="16" fill="#060A14" stroke="#EF4444" strokeWidth="2" />
              <circle cx="170" cy="70" r="16" fill="#060A14" stroke="#EF4444" strokeWidth="2" />
              <circle cx="70" cy="150" r="16" fill="#060A14" stroke="#EF4444" strokeWidth="2" />
              <circle cx="170" cy="150" r="16" fill="#060A14" stroke="#EF4444" strokeWidth="2" />
            </g>
          )}
        ...
        {selectedAnnotation && (
          <div className="absolute bottom-4 left-4 right-4 md:right-auto md:max-w-md bg-[#0F172A]/90 backdrop-blur border border-red-500/50 rounded-lg p-3 shadow-xl">
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-2">
                <span className="bg-red-950 text-red-400 border border-red-700/50 text-[10px] font-mono px-2 py-0.5 rounded font-bold">
